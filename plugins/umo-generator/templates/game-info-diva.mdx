...
export const CostumeClick = ({divaId}) => {
    ...
    const list = cos.map(k => {
      ...
      const colorImg = hasColor && (<><span style={{position:'absolute', maxWidth:'50%', right:'0', bottom:'0', backgroundColor:'#FFBBBBAA', borderRadius:'50%'}}><CostumeImage type="costume" costumeId={{id:id, color:1}} /></span></>);
      return (
