...
export const CostumeTextTable = () =>
{
  ...
  [
    ...
    {
      name:"Skill name",
      id:"cs_nm_"+(costumeInfo?.data?.HGHFFJKGNCO)?.padStart(4, '0'),
      bank:"master",
      params:[
        "Skill value"
      ]
    ...
    {
      name:"Skill desc",
      id:"cs_dsc_"+(costumeInfo?.data?.HGHFFJKGNCO)?.padStart(4, '0'),
      bank:"master",
      params:[
        "Skill value"
      ]
  ...
  const data_unlock = costumeInfo?.data.BJGNGNPHCBA.filter(c => c.INDDJNMPONH != "0").flatMap(c_ => 
    {
      const c = c_.INDDJNMPONH;
      if(c == "1")
      {
        return [
          {
      ...
      else if(c == "5")
      {
        return [
          ...
          },
          {
            ...
            id:"cos_reward_status_"+(c_.PIBLLGLCJEO[0]).padStart(4, '0'),
            bank:"master",
            order:53
          },
        ];
      ...
      else if(c == "6")
      {
        return [
          ...
          },
          {
            ...
            id:"cos_subplate_attr_"+(c_.PIBLLGLCJEO[0]).padStart(4, '0'),
            bank:"master",
            order:61
          },
          {
            name:"Unlock support plate status name",
            id:"cos_subplate_status_"+(c_.PIBLLGLCJEO[0]).padStart(4, '0'),
            bank:"master",
            order:62
          },
          {
  ...
  const dataCond = costumeInfo?.data?.BJGNGNPHCBA.filter(c => c.INDDJNMPONH != "0" && c.KBOLNIBLIND != "0").flatMap(c_ => 
    {
      const condList = data.database.costume.FDNBEPCEHBH.find(d => d.key == c_.KBOLNIBLIND).value;
      return condList.NKNBKLHCAFD.filter(d => d.INDDJNMPONH != "0").flatMap(d => {
        if(d.INDDJNMPONH == "1")
        {
          return [{
            name:"Unlock condition costume level",
            id:"cos_unlock_"+(d.INDDJNMPONH).padStart(4, '0'),
            bank:"master",
            params:[
              "Level to have",
...
{
  const costumeInfo = useContext(CostumeIdContext_);
  const levelPath = "CDENCMNHNGA/"+(costumeInfo.id - 1)+"/BJGNGNPHCBA/"+prop.index;
  switch(prop.level.INDDJNMPONH)
  {
    case "0":
      return "None";
    case "1":
      return (<>(<DatabaseValue database_name="costume" path={levelPath+'/INDDJNMPONH'} />)Costume Effect, skill level <DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/0'} /> </>);
    case "2":
    case "3":
      return <>(<DatabaseValue database_name="costume" path={levelPath+'/INDDJNMPONH'} />)Get Object : <DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/1'} />x <ObjectName itemId={prop.level.PIBLLGLCJEO[0]} />(<DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/0'} />)</>;
    case "4":
      return <>(<DatabaseValue database_name="costume" path={levelPath+'/INDDJNMPONH'} />)Unlock costume color <DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/0'} /></>;
    case "5":
      return <>(<DatabaseValue database_name="costume" path={levelPath+'/INDDJNMPONH'} />)Add stat : <StatBonus type={prop.level.PIBLLGLCJEO[0]} value={prop.level.PIBLLGLCJEO[1]} />(<DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/0'} />/<DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/1'} />)</>;
    case "6":
      return <>(<DatabaseValue database_name="costume" path={levelPath+'/INDDJNMPONH'} />)Unlock support plate <SupportPlate attr={prop.level.PIBLLGLCJEO[0]} status={prop.level.PIBLLGLCJEO[1]} />(<DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/0'} />/<DatabaseValue database_name="costume" path={levelPath+'/PIBLLGLCJEO/1'} />)</>;
  }
  return <></>;
}
...
export const UpgradeRow = (prop) =>
{
  ...
  const data = getPageData();
  const basePathPoints = "MBLNIECELNK/"+(cos.data.AGBPBDODKBK)+"/"+(prop.idx);
  const baseCostPoints = "AKKDOIJNMBH/"+(cos.data.AGBPBDODKBK)+"/"+(prop.idx);
  const unlockPoints = Object.entries(data.database.costume.MBLNIECELNK).find(([idx, c]) => c.key == cos.data.AGBPBDODKBK).value;
  const unlockCost = Object.entries(data.database.costume.AKKDOIJNMBH).find(([idx, c]) => c.key == cos.data.AGBPBDODKBK).value;
  return (<tr>
    <td>{prop.idx + 1}</td>
    <td><UpgradeRowUnlock index={prop.idx} level={prop.level} /></td>
    <td><UpgradeRowConds condIdx={prop.level.KBOLNIBLIND} /></td>
...
{
  const cos = useContext(CostumeIdContext_);
  const items = Object.entries(cos?.data?.BJGNGNPHCBA).filter(([idx, l]) => l.INDDJNMPONH != "0").map((l, idx) => (
...
export const CostumeImageWithColor = (p) =>
{
  ...
  const cos = getDatabaseValue("costume", "CDENCMNHNGA").find(c => c.JPIDIENBGKH_CostumeId == cosId?.id);
...
export const CostumeContentInContext = ({costumeId}) =>
{
  const props = {costumeId:costumeId};
  const data = getPageData();
...
* Id : <DatabaseValue database_name="costume" path={"CDENCMNHNGA/"+(props.costumeId - 1)+"/JPIDIENBGKH"} />
* Diva : <DatabaseValue database_name="costume" path={"CDENCMNHNGA/"+(props.costumeId - 1)+"/AHHJLDLAPAN"} /> <DivaLink divaId={getDatabaseValue("costume", "CDENCMNHNGA/"+(props.costumeId - 1)+"/AHHJLDLAPAN")} />
* Diva costume id : <DatabaseValue database_name="costume" path={"CDENCMNHNGA/"+(props.costumeId - 1)+"/DAJGPBLEEOB"} />
* Has special effect (wind and glow) : <DatabaseValue database_name="costume" path={"CDENCMNHNGA/"+(props.costumeId - 1)+"/EGLDFPILJLG"} />
...
export default function Content({data}) {
  ...
  return (
    <PageContext data={data}>
      <CostumeContentInContext costumeId={data.costumeId} />
    </PageContext>
  );
}