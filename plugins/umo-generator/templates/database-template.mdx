...
export function getReaders(iterator_owner, path, replace_)
{
    ...
    return readerList.map(reader => {
        ...
        for(var rp of splitReaderPath)
        {
            ...
            else if(readerRet.fb_struct?.[1].type == "string" && readerRet.fb_struct?.[1]?.items)
            {
                newRet.fb_struct = [rp, {...readerRet.fb_struct[1].items[0]}];
                newRet.fb_struct[1].reader_prop = rp;
                var listData = readerRet.fb_data.split(newRet.fb_struct["separator"]);
                if(listData[rp] == undefined)
                {    
                    //throw new Error("no data for "+rp+" in path "+path+" "+JSON.stringify(readerRet));
                    newRet.fb_error = "Array idx "+rp+" not in reader";
                }
                newRet.fb_data = listData[rp];
                newRet.fb_path += "/"+rp;
            }
            else
...
export function iterator_move(iterator, path)
{
    ...
    for(var p of splitPath)
    {
        ...
        if(prev.db_struct.type == "list")
        {
            ...
            {
                if(!prev.fb_struct.items && prev.db_struct.member_readerDisplay?.startsWith("ListString") && prev.fb_struct.type == "string")
                {
                    var sep = prev.db_struct.member_readerDisplay.split("|")[1];
                    prev.fb_struct.items = [{"type":"string", "separator":sep}];
                }
...
export const DbPropSource = ({member_data, key_}) =>
{
    ...
    return readers?.map(reader => {
        ...
        if(member_data.db_struct.member_display == "Date")
            subData += " ("+(new Date(parseInt(subData) * 1000).toLocaleString("en-GB", { timeZone: "UTC" }))+")";
...
export function getValueDisplay(value, member_it)
{
    ...
    {
        return value+" ("+(new Date(parseInt(value) * 1000).toLocaleString("en-GB", { timeZone: "UTC" }))+")";
    }
    return value;
}
...
export function generateNodesObject(iterator, prefix)
{
    return Object.entries(iterator.db_data).map(k => {
        ...
        const prefix_ = key+'-';
        const idx = k[0].indexOf('_');
        const varNames = idx == 11 ? [k[0].slice(0, idx), k[0].slice(idx + 1)] : [k[0]];
